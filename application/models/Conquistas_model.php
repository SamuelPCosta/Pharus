...
class Conquistas_model extends CI_Model {
	...
	public function conquista3($contaContrato){
		$this->db->select('faixa'); //Seleciona conta contrato
		$this->db->where('conta_contrato', $contaContrato); //Onde o usuário for igual ao nome de usuário q está logado
		$query = $this->db->get('usuario');
		$faixa = $query->row()->faixa;
		if ($faixa!=null) {//Se for >0 o usuário já tem simulacao
			$resposta="true"; //removi o true porque essa funcao e de teste
		}else{
			$resposta="false";
		}
		return $resposta;
