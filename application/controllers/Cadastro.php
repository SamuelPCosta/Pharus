...
class Cadastro extends CI_Controller {
	...
	public function carregarfornecedores(){
		$estado = $this->input->post('estado');
		if ($estado=='AC') {echo stripslashes(json_encode("<option value='Eletroacre'>Eletroacre</option>"));}
		elseif ($estado=='AL') {echo stripslashes(json_encode("<option value='Ceal'>Ceal</option>"));}
		elseif ($estado=='AP') {echo stripslashes(json_encode("<option value='AmE'>AmE</option>"));}
		elseif ($estado=='AM') {echo stripslashes(json_encode("<option value='CEA'>CEA</option>"));}
		elseif ($estado=='BA') {echo stripslashes(json_encode("<option value='Coelba'>Coelba</option>"));}
		elseif ($estado=='CE') {echo stripslashes(json_encode("<option value='Enel CE'>Enel CE</option>"));}
		elseif ($estado=='DF') {echo stripslashes(json_encode("<option value='CEB-DIS'>CEB-DIS</option>"));}
		elseif ($estado=='ES') {echo stripslashes(json_encode("<option value='EDP ES'>EDP ES</option><option value='ELSFM'>ELSFM</option>"));}
		elseif ($estado=='GO') {echo stripslashes(json_encode("<option value='Celg-D'>Celg-D</option><option value='Chesp'>Chesp</option>"));}
		elseif ($estado=='MA') {echo stripslashes(json_encode("<option value='Cemar'>Cemar</option>"));}
		elseif ($estado=='MT') {echo stripslashes(json_encode("<option value='EMS'>EMS</option>"));}
		elseif ($estado=='MS') {echo stripslashes(json_encode("<option value='EMT'>EMT</option>"));}
		elseif ($estado=='MG') {echo stripslashes(json_encode("<option value='Cemig-D'>Cemig-D</option><option value='DMED'>DMED</option><option value='EMG'>EMG</option>"));}
		elseif ($estado=='PA') {echo stripslashes(json_encode("<option value='Celpa'>Celpa</option>"));}
		elseif ($estado=='PB') {echo stripslashes(json_encode("<option value='EBO'>EBO</option><option value='EPB'>EPB</option>"));}
		elseif ($estado=='PR') {echo stripslashes(json_encode("<option value='CASTRO - DIS'>CASTRO - DIS</option><option value='Ceral DIS'>Ceral DIS</option><option value='Cocel'>Cocel</option><option value='Copel-DIS'>Copel-DIS</option><option value='Forcel'>Forcel</option>"));}
		elseif ($estado=='PE') {echo stripslashes(json_encode("<option value='Celpe'>Celpe</option>"));}
		elseif ($estado=='PI') {echo stripslashes(json_encode("<option value='Cepisa'>Cepisa</option>"));}
		elseif ($estado=='RJ') {echo stripslashes(json_encode("<option value='CERAL ARARUAMA'>CERAL ARARUAMA</option><option value='CERCI'>CERCI</option><option value='Ceres'>Ceres</option><option value='Enel RJ'>Enel RJ</option><option value='ENF'>ENF</option><option value='Light'>Light</option>"));}
		elseif ($estado=='RN') {echo stripslashes(json_encode("<option value='Cosern'>Cosern</option>"));}
		elseif ($estado=='RS') {echo stripslashes(json_encode("<option value='CEEE-D'>CEEE-D</option><option value='CERFOX'>CERFOX</option><option value='Ceriluz'>Ceriluz</option><option value='Cermissões'>Cermissões</option><option value='Certaja'>Certaja</option><option value='Certel'>Certel</option><option value='CERTHIL'>CERTHIL</option><option value='Cooperluz'>Cooperluz</option><option value='COOPERNORTE'>COOPERNORTE</option><option value='COOPERSUL'>COOPERSUL</option><option value='Coprel'>Coprel</option><option value='Creluz-D'>Creluz-D</option><option value='Creral'>Creral</option><option value='Demei'>Demei</option><option value='Eletrocar'>Eletrocar</option><option value='Hidropan'>Hidropan</option><option value='MuxEnergia'>MuxEnergia</option><option value='RGE (nova)'>RGE (nova)</option><option value='Uhenpal'>Uhenpal</option>"));}
		elseif ($estado=='RO') {echo stripslashes(json_encode("<option value='Ceron'>Ceron</option>"));}
		elseif ($estado=='RR') {echo stripslashes(json_encode("<option value='Roraima Energia'>Roraima Energia</option>"));}
		elseif ($estado=='SC') {echo stripslashes(json_encode("<option value='0.375'>Santa Catarina - CEGERO</option><option value='Cejama'>Cejama</option><option value='Celesc'>Celesc-DIS</option><option value='Ceprag'>Ceprag</option><option value='Ceraça'>Ceraça</option><option value='Ceral'>Ceral Anitápolis</option><option value='Cerbranorte'>Cerbranorte</option><option value='Cerej'>Cerej</option><option value='Cergal'>Cergal</option><option value='Cergapa'>Cergapa</option><option value='Cergral'>Cergral</option><option value='Cermoful'>Cermoful</option><option value='Cerpalo'>Cerpalo</option><option value='CERSAD'>CERSAD</option><option value='Cersul'>Cersul</option><option value='Certrel'>Certrel</option><option value='CODESAM'>CODESAM</option><option value='Coopera'>Coopera</option><option value='Cooperaliança'>Cooperaliança</option><option value='Coopercocal'>Coopercocal</option><option value='Coopermila'>Coopermila</option><option value='COOPERZEM'>COOPERZEM</option><option value='Coorsel'>Coorsel</option><option value='EFLJC'>EFLJC</option><option value='Eflul'>Eflul</option><option value='Lenergia'>Lenergia</option>"));}
		elseif ($estado=='SP') {echo stripslashes(json_encode("<option value='0.585'>São Paulo - Cedrap</option><option value='Cedri'>Cedri</option><option value='Cemirim'>Cemirim</option><option value='Cerim'>Cerim</option><option value='Ceripa'>Ceripa</option><option value='Ceris'>Ceris</option><option value='CERMC'>CERMC</option><option value='Cerpro'>Cerpro</option><option value='CERRP'>CERRP</option><option value='CERVAM'>CERVAM</option><option value='Cetril'>Cetril</option><option value='CPFL'>CPFL Paulista</option><option value='CPFL'>CPFL Piratininga</option><option value='CPFL'>CPFL Santa Cruz (nova)</option><option value='EDP'>EDP SP</option><option value='Elektro'>Elektro</option><option value='Eletropaulo'>Eletropaulo</option>"));}
		elseif ($estado=='SE') {echo stripslashes(json_encode("<option value='Cercos'>Cercos</option><option value='ESE'>ESE</option><option value='Sulgipe'>Sulgipe</option>"));}
		elseif ($estado=='TO') {echo stripslashes(json_encode("<option value='ETO'>ETO</option>"));}
	}
